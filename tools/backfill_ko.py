...
async def run(limit: int | None, dry: bool, sleep: float):
    store = Store(settings.db_path)
    rows = rows_to_move()
    todo = [r for r in rows if not store.is_seen(r["key"])]
    print(f"[이관] 원본 {len(rows)}건 · 남은 것 {len(todo)}건"
          + (f" · 이번에 {limit}건" if limit else ""))
    if limit:
        todo = todo[:limit]
    ...
    async with httpx.AsyncClient() as client:
        for i, r in enumerate(todo, 1):
            ...
            if not parts or missing(parts):
                print(f"  [건너뜀] 부품 부족: {r['headline'][:40]}")
                skipped += 1
                continue
            ...
            if not en or not en.get("headline"):
                print(f"  [실패] 번역: {r['headline'][:40]}")
                failed += 1
                continue

            # 한글·한자가 남았으면 올리지 않는다 — 영문 채널에 나가면 안 된다.
            body = " ".join(str(en.get(k, "")) for k in
                            ("headline", "lede", "comment")) + " ".join(en.get("bullets", []))
            # 괄호 안의 원어 병기는 정상이다(Niulai (牛来)). 괄호를 지우고 검사한다.
            if CJK.search(re.sub(r"\([^)]*\)", "", body)):
                print(f"  [실패] 번역에 원문 글자 잔존: {r['headline'][:40]}")
                failed += 1
                continue

            en["header_emoji"] = parts.get("header_emoji", "📰")
            cat = pipeline.normalize_category(r["category"])
            cat, why = country.enforce(cat, en)
            en["category"] = cat
            ...
            text = publisher.render(en, r["url"])
            if dry:
                print(f"\n{'='*62}\n[{cat}] {i}/{len(todo)}\n{'='*62}")
            ...
            tid = topics.thread_id_for(cat)
            mid = await publisher.send_raw(client, text, tid)
            if not mid:
                print(f"  [실패] 발행: {en['headline'][:40]}")
                failed += 1
                continue
            ...
            store.record_published(r["key"], mid, tid, r["url"], en["headline"],
                                   category=cat, lede=en.get("lede", ""),
                                   text=text, origin_at=r["ts"])
            ok += 1
            if ok % 10 == 0 or ok == 1:
                print(f"  [{ok}/{len(todo)}] {cat:<18} {en['headline'][:46]}")
            await asyncio.sleep(sleep)

    print(f"\n[이관] 완료 — 옮김 {ok} · 건너뜀 {skipped} · 실패 {failed}")
    print(f"       남은 것 {len([r for r in rows if not store.is_seen(r['key'])])}건")
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    asyncio.run(run(a.limit, a.dry_run, a.sleep))
